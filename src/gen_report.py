"""gen_report.py — 產生 report.docx（Phase 1-8 全管線驗收結果與觀察）"""
...
def main():
    ...
    # ── Phase 7 ──
    add_heading(doc, "Phase 7：覆蓋缺口分析與建議清單（s07_recommend.py，FR-08）", 2)
    add_para(doc, (
        "以 dist_any > 300m 為覆蓋缺口判斷標準，篩選未被任何固定式照相設備覆蓋的高風險熱點，"
        "依五年事故數排序並對映建議設備類型，輸出前 10 名建議清單。"
    ))
    add_table(doc,
        ["驗收條件", "預期值", "實際結果", "狀態"],
        [
            ["AC-08 Top1 地點", "中正區羅斯福路4段與基隆路4段口", "符合", "✓ 通過"],
            ["AC-08 Top1 件數", "539 件", "539 件", "✓ 通過"],
            ["AC-08 Top1 最近設備", "915 m", "915.4 m", "✓ 通過"],
            ["覆蓋缺口熱點數", "—", "87 個（dist_any > 300m）", "✓"],
            ["已覆蓋熱點數", "—", "58 個（dist_any ≤ 300m）", "✓"],
            ["recommendations.csv 輸出", "前 10 名", "10 筆，18 欄", "✓ 通過"],
        ]
    )
    doc.add_paragraph()
    add_para(doc, "前 10 名建議清單（依五年事故數排序）：")
    add_table(doc,
        ["排名", "地點", "件數", "A1", "最近設備(m)", "pred_2025", "型態", "建議設備類型"],
        [
            ["1",  "中正區羅斯福路4段與基隆路4段口",      "539", "1", "915",  "121件", "全日型",    "綜合評估（A1加權）"],
            ["2",  "大安區新生南路與信義路口",             "469", "1", "380",  "98件",  "全日型",    "綜合評估（A1加權）"],
            ["3",  "信義區忠孝東路與光復南路口",           "415", "0", "469",  "85件",  "全日型",    "綜合評估（A1加權）"],
            ["4",  "士林區中山北路2段與德行路口",          "350", "3", "656",  "73件",  "全日型",    "綜合評估（A1加權）"],
            ["5",  "松山區敦化北路5段與民族東路口",        "289", "1", "460",  "61件",  "全日型",    "綜合評估（A1加權）"],
            ["6",  "大安區和平東路與復興南路口",           "288", "1", "831",  "62件",  "假日型",    "行人安全科技執法"],
            ["7",  "中山區中山北路2段與長安東路口",        "279", "0", "496",  "58件",  "假日型",    "行人安全科技執法"],
            ["8",  "中山區中山北路2段與林森北路口",        "276", "0", "352",  "60件",  "全日型",    "綜合評估（A1加權）"],
            ["9",  "大安區信義路4段與基隆路1段口",         "244", "0", "317",  "50件",  "全日型",    "綜合評估（A1加權）"],
            ["10", "信義區信義路2段與林森南路口",          "237", "0", "658",  "55件",  "全日型",    "綜合評估（A1加權）"],
        ]
    )
    doc.add_paragraph()
    add_para(doc, (
        "【觀察一】前 10 名中有 8 個為「全日型」熱點，2 個為「假日型」，"
        "主因這兩種型態事故數高、設備覆蓋率低，夜間型與通勤型熱點多已有設備覆蓋。"
    ))
    add_para(doc, (
        "【觀察二】pred_2025（XGBoost 預測 2025 年總件數）顯示第 1 名預估仍有 121 件，"
        "與歷年趨勢一致，建議優先評估設置。"
    ))
    add_para(doc, (
        "【限制說明（NFR-5）】本清單排序依「五年事故絕對數量」，"
        "未以車流量正規化，屬絕對風險排序而非相對風險率，報告中需明確說明。"
    ))

    # ── Phase 8 ──
    add_heading(doc, "Phase 8：視覺化圖表產出（s08_visualize.py，FR-09）", 2)
    add_para(doc, (
        "產出 6 張 PNG 圖表（≥150 dpi）與 1 份 HTML 互動地圖，"
        "涵蓋空間分布、時間序列、PCA分群、預測比較等分析面向。"
        "中文字型使用 Microsoft JhengHei，確保無豆腐字。"
    ))
    add_table(doc,
        ["圖表", "檔案", "大小", "解析度", "說明", "狀態"],
        [
            ["F1 空間密度+熱點", "F1.png",  "360 KB", "1537×1482",
             "hexbin 事故密度底圖 + 熱點散點（型態著色，大小∝件數）", "✓"],
            ["F2 互動地圖",      "F2.html", "391 KB", "—",
             "folium 互動：熱點圓圈 + 設備標記 + Top10缺口高亮", "✓"],
            ["F2 靜態版",        "F2.png",  "147 KB", "1637×1486",
             "matplotlib 靜態版，含設備三角標記與排名標注", "✓"],
            ["F3 A1/A2比例",     "F3.png",   "90 KB", "1936×917",
             "各年堆疊長條（A1/A2件數）+ A1占比折線", "✓"],
            ["F4 日事故序列",    "F4.png",  "253 KB", "2387×734",
             "全市日序列折線 + 30日MA + COVID灰底 + 訓練/驗證切分線", "✓"],
            ["F5 PCA+群曲線",    "F5.png",  "178 KB", "1400×1055",
             "PC1 vs PC2 散佈（型態著色）+ 4種型態平均24hr曲線", "✓"],
            ["F6 預測比較",      "F6.png",  "572 KB", "2387×884",
             "2025驗證期：Naive/SARIMA/XGBoost vs 實際，含指標文字框", "✓"],
        ]
    )
    doc.add_paragraph()
    add_table(doc,
        ["驗收條件", "預期值", "實際結果", "狀態"],
        [
            ["AC-09 圖檔輸出至 figures/", "✓", "7 個檔案全部輸出", "✓ 通過"],
            ["AC-09 PNG ≥ 150 dpi", "≥ 150 dpi", "全部使用 DPI=150 儲存", "✓ 通過"],
            ["AC-09 含標題與軸標籤", "✓", "全部圖表含中文標題與軸標籤", "✓ 通過"],
            ["中文字型無亂碼", "✓", "Microsoft JhengHei，無豆腐字", "✓ 通過"],
            ["F2 HTML 互動地圖", "✓", "F2.html（folium，391 KB）", "✓ 通過"],
        ]
    )
    doc.add_paragraph()
    add_para(doc, (
        "【F4 觀察】2021-05-19~07-26 COVID-19 三級警戒期間事故數明顯低於前後，"
        "灰底標注區域清楚呈現此結構性斷點。30 日移動平均線顯示 2022 年後事故數有緩降趨勢。"
    ))
    add_para(doc, (
        "【F6 觀察】2025 驗證期中 SARIMA 與 XGBoost 預測線高度重疊，"
        "兩者均能捕捉日事故數的週期波動，但在年末假期（2025-12 月）"
        "均有輕微低估的現象，為可能的改善方向。"
    ))

    # ══ 三、驗收總覽 ══
    add_heading(doc, "三、驗收總覽（全管線）", 1)
    add_table(doc,
        ["Phase", "模組", "對應SRS", "關鍵驗收", "狀態"],
        [
            ["Phase 1", "s01_clean.py",    "FR-01", "118,980筆；anomalies.csv（3筆）",    "✓ 通過"],
            ["Phase 2", "s02_hotspot.py",  "FR-02", "145熱點；19.2%；Top1=539件/915m",   "✓ 通過"],
            ["Phase 3", "s03_series.py",   "FR-03", "1826天無缺日；145×60；145×43",       "✓ 通過"],
            ["Phase 4", "s04_features.py", "FR-04", "NaN=0；T-4無洩漏；COVID dummy=69天", "✓ 通過"],
            ["Phase 5", "s05_pca_kmeans",  "FR-05/06","k=8；145熱點皆有標籤；4種型態",   "✓通過（PCA討論）"],
            ["Phase 6", "s06_forecast.py", "FR-07", "XGBoost+25.4%；SARIMA+26.4%",       "✓ 通過"],
            ["Phase 7", "s07_recommend.py","FR-08", "AC-08 Top1 539件/915m；87個缺口",    "✓ 通過"],
            ["Phase 8", "s08_visualize.py","FR-09", "F1~F6+HTML；≥150dpi；中文無亂碼",    "✓ 通過"],
        ]
    )
    doc.add_paragraph()
    add_para(doc, (
        "全管線（Phase 1–8）共 8 個主要模組全部通過驗收，"
        "總執行時間（不含 SARIMA 網格搜尋）< 2 分鐘，符合 NFR-2（10 分鐘）要求。"
        "SARIMA 網格搜尋（36 組）耗時約 20 秒，含 SARIMA 的完整管線約 25 秒。"
    ))

    # ══ 四、報告撰寫注意事項 ══
    add_heading(doc, "四、報告撰寫注意事項（SRS 規範）", 1)
    notes = [
